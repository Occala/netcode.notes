...
There's 1 byte of header denoting the data type, I implemented doubles and decided I'd support up to 8 bytes of data. 1 byte was reserved for denoting the bytes as atomically ready to push (because osc sends are not atomic, we change the header, then the data bytes, then flag the send as valid with the trailing byte).
