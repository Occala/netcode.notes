...
If you have a compatible world and avatar setup, you can push data in either direction. To test this, I sent OSC data from a unity scene to an avatar, my corresponding world then read the avatar's input bytes. This sent around 10 bytes per command and required very explicit acks to align timing.
...
It follows a similar origin reading scheme for avatar senders and world receivers, but doesn't push bytes. Proximity acts directly as the normalized value for both sliders. The mute toggle is simply a bool receiver instead
