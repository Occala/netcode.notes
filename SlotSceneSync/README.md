...
For the sake of data saving, a byte length is not prefixed to each entry. This is less safe, but saves on data across all available slots. A runtime error will not purge data at least and logic shouldn't continue regardless if something fails to read as a result of errors in deserialization
