...
In regards to latency, if someone dropped frames or drops packets, they may end up spawning cubes on top of each other. This kind of ruins the local simulation, it may be preferable to queue spawns in some way or resimulate them for the amount of time lost, but it'd be more complex (a queue/consumption style for pooled spawns would probably be the best option there)
